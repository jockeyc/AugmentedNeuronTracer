        ...
        Pass
        {
            ...
            float4 Frag(VaryingsDefault i) : SV_Target
            {
                ...
                if(dstLimit>0)
                {
                    ...
                    //    color = float4(base + (1-result.a)*color.rgb,1);
                    //}
                    if(result.b >= result.r) 
                    {
                        if(result.b<0.1)
                        {
                            float3 base = lerp(float3(1,1,1),float3(0.5,0.7,0.9),(result.b)/0.1);
                            color = float4(result.b * base + (1-result.b)*color.rgb,1);
                        }
                        else if(result.b>0.1 && result.b<0.5)
                        {
                            float3 base = lerp(float3(0.5,0.7,0.9),float3(0.6,0.7,1),(result.b-0.1)/0.4);
                            color = float4(result.b * base + (1-result.b)*color.rgb,1);
                        }
                        else
                        {
                            float3 base = lerp(float3(0.6,0.7,1),float3(1,1,1),(result.b-0.5)/0.5);
                            color = float4(result.b * base + (1-result.b)*color.rgb,1);
                        }
                        //float3 base = float3(result.b,result.b,result.b);
                        //color = float4(base + (1-result.a)*color.rgb,1);
                    }
                    else 
                    {
                        float3 base = float3(1*result.r,1*result.r,1*result.r);
                        color = float4(base + (1-result.a)*color.rgb,1);
                    }
                    //if(color.r>0||color.b>0||color.g>0)s
                    //{
