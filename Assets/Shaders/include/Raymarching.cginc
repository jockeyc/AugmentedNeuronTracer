...
float2 solveCubic2(float3 a)
{
    ...
    if (d > .0)
    {
        ...
        return float2(addv(sign(x) * pow(abs(x), float2(1. / 3., 1. / 3.))) - a.x / 3.,
                      addv(sign(x) * pow(abs(x), float2(1. / 3., 1. / 3.))) -
                      a.x / 3.);
    }
...
float SdfBezier(float3 p, float4 a, float4 b, float4 c, float bias=0)
{
    float h = 1e-5;
     b.xyz += lerp(float3(h, h, h), float3(0, 0, 0), abs(sign(b.xyz * 2. - a.xyz - c.xyz)));
    //b.xyz += sign(b.xyz * 2.0 - a.xyz - c.xyz) * 1e-6;
    float3 A = b - a;
    float3 B = c - b - A;
    float3 C = p - a;
    float3 D = A * 2.;
    ...
    if (t <= 0)
    {
        ...
        if(bias>0)
        {
            ...
            if(radius_A==radius_B)
            {
                ...
                if(dot(p-a.xyz,line_dir)>bias) dist = distance(p,pos_leaf)-radius_A;
                else
                {
                    float3 projection = dot(p-a.xyz, line_dir) * line_dir;
                    float3 perpVector = p-a.xyz - projection;
            ...
            else
            {
                ...
                float3 perpVector = p-a.xyz - projection;
                if(length(projection) > limit) dist = distance(p,pos_leaf) - tan_radius;
                else dist = length(perpVector) -  radius;
            }
        }
        else dist = DistancePointToDisk(p,a,normalize(a-b),radius_A);
    }
    else if(t>=1) dist = DistancePointToDisk(p,c,normalize(c-b),c.w);
    return dist;
}
